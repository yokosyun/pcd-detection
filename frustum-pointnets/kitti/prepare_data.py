...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    pos_cnt = 0
    all_cnt = 0
    for data_idx in range(464) :#data_idx_list:
...
def extract_frustum_data_rgb_detection(det_filename, split, output_filename,
    ...
    for det_idx in range(464) :#range(len(det_id_list)):
        ...
        box_fov_inds = box_fov_inds & img_fov_inds
        pc_in_box_fov = pc_rect[box_fov_inds,:]
        tmp_pc_in_img_fov = pc_rect[img_fov_inds,:]
        # Get frustum angle (according to center pixel in 2D BOX)
        box2d_center = np.array([(xmin+xmax)/2.0, (ymin+ymax)/2.0])
        uvdepth = np.zeros((1,3))
        uvdepth[0,0:2] = box2d_center
        ...
        input_list.append(pc_in_box_fov)
        tmp_input_list.append(tmp_pc_in_img_fov)
        frustum_angle_list.append(frustum_angle)
    ...
    if viz:
        import mayavi.mlab as mlab
        for i in range(10):
            tmp_pc = tmp_input_list[i]
            p1 = input_list[i]
            print(p1[:,0].size)

            point_size = [1000] * p1[:,0].size
            #point_size_tmp = [1000] * tmp_pc[:,0].size
            point_size = np.random.random((1, p1[:,0].size))

            fig = mlab.figure(figure=None, bgcolor=(0.8,0.4,0.4),
                fgcolor=None, engine=None, size=(1000, 1000))
            mlab.points3d(tmp_pc[:,2], -tmp_pc[:,0], -tmp_pc[:,1], mode='point',
                colormap='gnuplot',color=(0,0,0), scale_factor=1, figure=fig)

            mlab.points3d(p1[:,2], -p1[:,0], -p1[:,1] ,color=(0,1,0), mode='point',
                colormap='gnuplot', scale_factor=1, figure=fig)
            raw_input()
